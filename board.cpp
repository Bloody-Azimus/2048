...
void Board::initRandomTile()
{
    ...
    if (tileValue <= valueThr) tileBoard[tileRow][tileCol] = new Tile(2);
    else tileBoard[tileRow][tileCol] = new Tile(4);
}
...
void Board::moveVertically(int row, int col, Direction dir)
{
    if (tileBoard[row][col] != NULL) {
        ...
        int newRow;
        if (dir == UP)
            newRow = row - 1; // двигает вверх
        else
            newRow = row + 1; // двигает вниз

        // Двигаемся, пока плитка не достигнет края доски или не будет найдена другая плитка
        ...
        // Если плитка сойдет с доски, останавливаем ее
        ...
        // Если есть столкновение с другой плиткой, проверим значение этой плитки
        else
        {
            // Если плитка имеет то же значение, обновляем плитку
            ...
            // В противном случае помещаем движущуюся плитку рядом с ней.
            else {
                if (dir == UP)
...
void Board::moveHorizontally(int row, int col, Direction dir)
{
    if (tileBoard[row][col] != NULL)
    {
        ...
        int newCol;
        if (dir == RIGHT)
            newCol = col + 1; // Двигаем вправо
        else
            newCol = col - 1; // Двигаем влево
        ...
        // Если плитка сойдет с доски, остановим ее
        ...
        {
            // Если плитка имеет то же значение, обновим плитку
            ...
            // В противном случае поместим движущуюся плитку рядом с ней.
            else {
                if (dir == RIGHT)
...
void Board::move(Direction dir)
{
    ...
    // Если доска изменилась, поместим новую случайную плитку
...
bool Board::canMove() const
{
    if (isFull())
    {
        // Пробуем сделать новый ход на копии доски
        Board newBoard(*this);
        newBoard.move(UP);
        ...
        // Если все вышеперечисленное не удалось, то движение невозможно
        return false;
    }
    else
    {
